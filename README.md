...
**Formas a mais ou mal classificadas.** A oclusão do objeto sobre um marcador nem sempre apaga a forma: às vezes ela ganha ou perde um vértice e é detectada como o tipo errado. Estes casos são tratados antes de decidir se o quadro tem 4, 3 ou menos marcadores utilizáveis:

- **Forma a mais** (ex.: 2 triângulos + 3 quadrados): entre as combinações de 2 e 2, fica a que deixa a razão entre as distâncias (quadrados)/(triângulos) mais perto da mediana dos outros quadros; as formas descartadas aparecem marcadas em cinza na foto anotada ([`MarkerDetector.reduce`](src/main/java/scan3d/MarkerDetector.java)).
- **Forma mal classificada** (1 triângulo + 3 quadrados, ou o inverso): um dos "quadrados" é, na verdade, o triângulo que a oclusão deformou. Entre os candidatos cuja razão de distâncias fica dentro da tolerância, fica o de **menor raio** — o marcador ocluído tende a aparecer menor (um contorno fragmentado), enquanto os outros candidatos costumam ser só os marcadores de verdade ([`MarkerDetector.reclassify`](src/main/java/scan3d/MarkerDetector.java)).
...
4. **Silhueta externa.** Para cada faixa de altura, consideram-se apenas os pontos extremos, esquerdo e direito, do contorno. Bordas internas (como as do rótulo) são descartadas, assim como picos isolados comparados às linhas vizinhas (por exemplo, a borda da folha em contato com o objeto) e linhas cujo raio destoa muito da mediana de **todas** as lâminas — este segundo filtro pega os casos em que a borda da folha vaza numa faixa inteira de altura, contígua o bastante para não destoar dos vizinhos imediatos. O topo e a base do objeto são deliberadamente excluídos, pois, com a câmera posicionada acima do objeto, as elipses correspondentes a essas regiões aparecem deslocadas em altura.
...
Nem sempre os quatro marcadores são visíveis simultaneamente: um deles pode estar oculto pelo objeto, fora do enquadramento ou desfocado. Em vez de descartar tais quadros, o sistema procura recuperar a pose a partir de **três marcadores**, atribuindo-lhes o rótulo de confiança reduzida e um **nível** de 1 a 3 conforme a qualidade da recuperação. Quadros sem, ao menos, três marcadores utilizáveis são descartados, com o motivo correspondente registrado no log e no arquivo `skipped.txt`.

A utilização de apenas três pontos introduz duas fontes de ambiguidade: a identidade ("0" ou "1") do marcador de um tipo que aparece uma única vez, e a multiplicidade de soluções do problema P3P (*Perspective-3-Point*), que pode retornar até quatro soluções distintas. O sistema testa exaustivamente todas as hipóteses e seleciona a pose mais próxima à de um **quadro vizinho com quatro marcadores** — em posição **e** rotação, para não aceitar uma hipótese com posição parecida mas orientação bem diferente —, descartando soluções fisicamente inválidas. Os detalhes de implementação constam em [`PoseEstimator.estimateFromThree`](src/main/java/scan3d/PoseEstimator.java) e [`Pipeline`](src/main/java/scan3d/Pipeline.java):

- **Nível 1**: quatro marcadores, detectados diretamente ou recuperados de formas a mais/mal classificadas (seção anterior).
- **Nível 2**: três marcadores, com um quadro de referência a, no máximo, `--max-gap` quadros de distância (padrão: 3) e pose que não excede 60 mm por quadro de distância e 30° de diferença angular em relação a ele. Quadros de nível 2 já resolvidos podem servir de referência para os seguintes, em camadas sucessivas, até 8 saltos; como cada pose deriva de uma solução P3P independente — a referência só desempata —, o erro não se acumula ao longo da cadeia.
- **Nível 3**: três marcadores, mas sem quadro de referência confiável dentro de `--max-gap`, ou com salto/rotação acima do limite do nível 2. Em vez de descartar, o sistema tenta de novo sem esses limites, usando o quadro com pose mais próximo (de qualquer nível) como referência; só desiste se não houver pose nenhuma na sequência ou se nenhuma hipótese do P3P for fisicamente válida.

Não há erro de reprojeção associado a soluções com três pontos, uma vez que o ajuste é exato; nesse caso, a coluna correspondente permanece vazia, sendo substituída pela distância até a referência. A pose determina, também, a região da imagem em que o objeto pode se encontrar: o contorno é considerado válido apenas dentro do volume delimitado pelo retângulo dos marcadores reprojetado segundo essa pose, o que permanece válido mesmo quando um dos marcadores está oculto.
...
Das 129 fotografias de exemplo, obtêm-se 78 quadros de nível 1, 47 de nível 2 e 1 de nível 3, totalizando **126 quadros aproveitados (98%)**; restam 3 quadros com só 2 marcadores, que não é possível resolver. Para restringir o processamento apenas a quadros de nível 1, utiliza-se a opção `--min-markers 4`.

Nas saídas do sistema, o nível de confiança é registrado nas colunas `markers`/`confidence`/`level` de `poses.csv` e na propriedade `markers` de `cloud.ply` (que só distingue 3 de 4: níveis 2 e 3 aparecem como 3). Quadros de confiança reduzida (níveis 2 e 3) são destacados em **laranja**, com aviso correspondente, tanto na fotografia anotada quanto na interface gráfica.
...
| Etapa | Estado |
| --- | --- |
| Detecção e ordem dos marcadores | ✅ 78 quadros de nível 1 (4 marcadores, diretos ou recuperados de formas a mais/mal classificadas); ordem consistente nos dois lados da volta (24 rotações sintéticas + conferência visual) |
| Quadros com 3 marcadores | ✅ 48 recuperados (47 de nível 2, 1 de nível 3), marcados como confiança menor (validação e limites em [Quadros com 3 marcadores](#quadros-com-3-marcadores)) |
| Quadros ignorados | 3 de 129, todos com só 2 marcadores utilizáveis (1 triângulo + 1 quadrado); motivo registrado em `skipped.txt` |
| Pose | ✅ erro de reprojeção mediano 1,2 px (câmera aproximada); ⚠️ 26 px com o `TextMatrix.txt` original |
| Contorno do objeto | ✅ presente em todos os 126 quadros, contínuo na maioria (mediana de ~1,4 mil pixels); ⚠️ com a borda de trás da folha e detalhes do rótulo em alguns quadros |
| Nuvem de pontos | ✅ 39.240 pontos com a forma do pote, raio estável (dispersão de 0,6 a 1,8 mm), cobertura de 302° de 360°; ⚠️ aproximação para objetos que não são de revolução |
| Volta de 360° | ✅ o ângulo `rz` da pose cobre −179° a +180° |
| Interface gráfica e visualizador | ✅ processar, visualizar, navegar quadros e contornos (testado via captura de tela; a interação direta do usuário com os botões não foi exercitada pelo autor) |
...
| `output/poses.csv` | por quadro: `markers` (4 ou 3), `confidence` (`high`/`low`) e `level` (1, 2 ou 3 — ver [Quadros com 3 marcadores](#quadros-com-3-marcadores)), `tx, ty, tz` (mm), `rx, ry, rz` (graus), erro de reprojeção (px; vazio com 3 marcadores), `ref_frame`, `ref_hops`, `ref_dist_mm` (só com 3) e nº de pontos |
...
2. **Visual hull** (interseção de silhuetas estendidas) para objetos que não são sólidos de revolução, uma vez que o método de lâminas infla faces planas e não captura concavidades. Depende da obtenção de silhuetas preenchidas (item 3). Uma tentativa mais simples — ligar por triângulos os pontos de lâminas vizinhas, sem passar pelo visual hull — foi implementada e descartada: o contorno tem ruído localizado (a borda da folha, item 3) que uma malha conecta em picos bem visíveis, algo que a nuvem de pontos tolera bem melhor. Resolver isso passa por segmentar a silhueta primeiro (item 3), não por filtrar a malha depois.
...
4. Filtragem ou ponderação dos pontos provenientes de quadros de confiança reduzida na etapa de reconstrução, com avaliação do efeito correspondente em fotografias reais.
5. Portabilidade para Android (OpenCV Android, mantendo a mesma lógica de processamento), a ser realizada após a validação completa do equacionamento matemático.
